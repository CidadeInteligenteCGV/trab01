...
O sistema proposto para o "Controle de Frequência de Alunos" conterá as informações aqui detalhadas. Dos alunos serão armazenados o código de matrícula, nome, CPF, data de nascimento, sexo, campus e curso. Cada aluno deve estar alocado, obrigatoriamente a um único campus, que terá um código, a cidade onde se encontra e as disciplinas que oferta. Das disciplinas serão armazenados seu código, nome, curso que a está ofertando, nome do professor que vai lecioná-la e sua carga horária. Cada disciplina gera um histórico de aula único, em que consta a data da aula, hora de início e fim, carga horária mínima obrigatória, a frequência dos alunos na aula e a carga horária que eles cumpriram. Deve-se destacar que cada aluno pode estar frequentando várias disciplinas, e cada disciplina pode possuir vários alunos matriculados, porém cada disciplina só deve possuir um histórico de aula. Os dados relativos aos professores que serão armazenados são código do servidor, nome, matérias que leciona e CPF. É importante destacar que cada professor pode lecionar mais de uma disciplina e pode possuir mais de um campus como área de trabalho. Com relação ao histórico de frequência do aluno devemos armazenar as informações de data da aula, hora de chegada e saída do aluno, a carga horária mínima obrigatória, a frequência nas aulas e a carga horária que cumpriu. É importante lembrar que cada aluno pode ter diversos eventos de histórico de frequência associados às matérias que está matroculado durante o período letivo, com a atualização dos dados por aula.

O Sistema consiste no Controle de Frequência de Aulas, e tem como objetivo principal, registrar a frequências dos alunos durante o período letivo. Sua principais funções são: controle de entrada e saída do aluno na sala de aula, e gerar relatórios de frequências e médias de evasão.
Ao se matricular na instituição, o aluno será orientado a baixar o aplicativo que dá acesso a suas informações acadêmicas e realizar a liberação ID para utilização de QRCode disponibilizado para smartphones. Dentro de 5 dias e antes do ínicio das aulas, o aluno recebe a carteirinha do estudante, esta carteirinha além de conter os seus dados pessoais, vem com um micro chip que se comunica com o sistema e servidores da instituição. 
De acordo com a disciplina matriculada, o aluno deverá se deslocar até a sala correta onde a disciplina será lecionada e para ter acesso a sala de aula e registrar sua frequência, o mesmo deverá posicionar sua carteirinha ou QRcode no leitor localizada na entrada da sala de aula. O aluno poderá sair da sala quantos vezes achar necessário e precisar, porém para que o sistema registre sua frequência é necessário que sua última saída registrada seja dentro dos últimos 15 minutos de aula, esta mesma regra também se aplica no ao início da aula, o aluno disponibiliza de até 15 minutos de tolerância para que o sistema processe sua frequência (entrada).  
O sistema deverá guardar o status dos alunos mesmo se a rede estiver off-line, até que seja possível fazer seu envio para o banco de dados on-line, e assim, poder gerar um relatório de frequência.
O aluno pode optar por utilizar tanto a carteirinha de estudante quanto o QRcode, sendo necessário apenas uma das modalidades.
Caso haja extravio de carteirinha, o aluno deverá comunicar o registro acadêmico para que seja confeccionada uma nova carteirinha. Se o aluno for desligado do seu curso por qualquer motivo, automaticamente o seu QRcode é desativado. Caso ocorra também o extravio do smartphone portador ID de acesso do aluno, o mesmo deve comunicar a instituição para que haja bloqueio da IDE do aparelho e uma nova liberação.
